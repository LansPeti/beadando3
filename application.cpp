...
public:
    ApplicationImpl()
        : current_state(State::MAIN_MENU),
          main_menu(new MainMenu(0, 0, 1000, 600, [this]() { start_game(false); }, [this]() { start_game(true); })),
          game(nullptr), pause_menu(nullptr), game_over_menu(nullptr), board_size(15) {}
    ...
    void start_game(bool bot) {
        ...
        delete game;
        game = new Game(0, 0, board_size * 20, &master, [this]() { pause_game(); }, [this](const std::string& winner) { game_over(winner); }, bot);
        current_state = State::GAME;
    }

    void pause_game() {
        if (pause_menu == nullptr) {
            pause_menu = new PauseMenu(0, 0, 1000, 600, [this]() { resume_game(); });
        }
    ...
    void game_over(const std::string& winner) {
        if (game_over_menu == nullptr) {
            game_over_menu = new GameOverMenu(0, 0, 1000, 600, winner, [this]() { restart_game(); }, [this]() { go_to_main_menu(); });
        }
